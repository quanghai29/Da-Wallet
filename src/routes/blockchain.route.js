...
router.get('/transactionPool', (req, res) => {
    const transactionPool = trsPool.getTransactionPool();
    if(transactionPool === undefined){
        return res.status(400).json({message: 'Not Found txPool'})
    }
...
router.post('/mineBlockWithTransaction', (req, res) => {
    ...
    try {
        const resp = bitcoin.generatenextBlockWithTransaction(privateKey, address, receiveAddress, amount);
        res.send(resp);
    } catch (e) {
        console.log(e.message);
...
router.post('/myUnspentTransactionOutputs', (req, res) => {
    const address = req.body.address;
    res.status(200).json(bitcoin.getMyUnspentTransactionOutputs(address));
});
/////////////////////////////////
